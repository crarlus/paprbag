...
GetBestTranslatedSequence <- function(Reads){
  ...
  Reads_subseqs_f <- lapply(1:3, function(pos)  subseq(Reads, start=pos))
  Reads_subseqs_r <- lapply(1:3, function(pos)  subseq(reverseComplement(Reads), start=pos))

  Reads_TranslatedSeq_bs_3frame_f <- lapply(Reads_subseqs_f, function(seq) Biostrings::translate(seq,if.fuzzy.codon="solve",genetic.code = getGeneticCode("11")) )
  Reads_TranslatedSeq_bs_3frame_r <- lapply(Reads_subseqs_r, function(seq) Biostrings::translate(seq,if.fuzzy.codon="solve",genetic.code = getGeneticCode("11")) )

  AAfreq0f <- alphabetFrequency(Reads_TranslatedSeq_bs_3frame_f[[1]], as.prob=F)
  AAfreq1f <- alphabetFrequency(Reads_TranslatedSeq_bs_3frame_f[[2]], as.prob=F)
  AAfreq2f <- alphabetFrequency(Reads_TranslatedSeq_bs_3frame_f[[3]], as.prob=F)
  AAfreq0r <- alphabetFrequency(Reads_TranslatedSeq_bs_3frame_r[[1]], as.prob=F)
  AAfreq1r <- alphabetFrequency(Reads_TranslatedSeq_bs_3frame_r[[2]], as.prob=F)
  AAfreq2r <- alphabetFrequency(Reads_TranslatedSeq_bs_3frame_r[[3]], as.prob=F)
